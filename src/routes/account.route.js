...
export default function createAccountRouter(container) {
  ...
  const cryptoService = container ? container.resolve("CryptoProvider") : null;
  const userModelDb = container ? container.resolve("UserModel") : userModel;
  ...
  async function generateAndSendOtp(user, purpose, subject, emailTemplateText) {
    ...
    return otp; // Return otp is sometimes useful, e.g in signup where it sends verification email with link
  }
  ...
  router.post("/reset-password", async (req, res) => {
    ...
    const hashedPassword = cryptoService
      ? await cryptoService.hash(new_password)
  ...
  router.post("/signin", async function (req, res) {
    ...
    const isPasswordValid = cryptoService
      ? await cryptoService.compare(password, user.password_hash)
  ...
  router.put("/profile", isAuthenticated, async (req, res) => {
    try {
      ...
      const entity = identityProvider.prepareUpdateEntity(
        currentUser,
        req.body,
      );
  ...
  router.get("/request-upgrade", isAuthenticated, async (req, res) => {
    ...
    const upgradeRequest =
      await upgradeRequestModel.findByUserId(currentUserId);
