...
import * as sellerAnalyticsModel from "../models/seller.analytics.model.js";
...
router.get("/", async function (req, res) {
  ...
  const stats = await sellerAnalyticsModel.getSellerStats(sellerId);
...
router.get("/products", async function (req, res) {
  const sellerId = req.session.authUser.id;
  const products =
    await sellerAnalyticsModel.findAllProductsBySellerId(sellerId);
...
router.get("/products/active", async function (req, res) {
  const sellerId = req.session.authUser.id;
  const products =
    await sellerAnalyticsModel.findActiveProductsBySellerId(sellerId);
...
router.get("/products/pending", async function (req, res) {
  ...
  const [products, stats] = await Promise.all([
    sellerAnalyticsModel.findPendingProductsBySellerId(sellerId),
    sellerAnalyticsModel.getPendingProductsStats(sellerId),
  ]);
...
router.get("/products/sold", async function (req, res) {
  const sellerId = req.session.authUser.id;
  const [products, stats] = await Promise.all([
    sellerAnalyticsModel.findSoldProductsBySellerId(sellerId),
    sellerAnalyticsModel.getSoldProductsStats(sellerId),
  ]);
...
router.get("/products/expired", async function (req, res) {
  const sellerId = req.session.authUser.id;
  const products =
    await sellerAnalyticsModel.findExpiredProductsBySellerId(sellerId);
